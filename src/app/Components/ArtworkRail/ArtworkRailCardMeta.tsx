...
export const ArtworkRailCardMeta: React.FC<ArtworkRailCardMetaProps> = ({
  ...
  // TODO: use handleFollowToggle from handleFollowToggle Artist/useFollowArtist instead of commitMutation
  ...
  const ArtworkCardCTAWrapper: React.FC<{ onPress?(): any }> = ({ onPress, children }) => {
    return (
      <Touchable
        haptic
        onPress={onPress}
  ...
  const renderNewSaveCTA = () => {
    return (
      <ArtworkCardCTAWrapper onPress={saveArtworkToLists}>
        {isSaved ? (
          <NewFillHeartIcon
            testID="filled-heart-icon"
  ...
  // TODO: add artist followed banner
  const renderFollowCTA = () => {
    return (
      <ArtworkCardCTAWrapper onPress={handleOnFollow}>
        {artist?.isFollowed ? (
          <FollowArtistFillIcon
            testID="filled-follow-icon"
            height={ARTWORK_RAIL_CARD_CTA_ICON_SIZE}
            width={ARTWORK_RAIL_CARD_CTA_ICON_SIZE}
            fill="black100"
          />
  ...
  return (
    <Flex my={1}>
      ...
      >
        ...
        {!!showSaveIcon && !enableRedesignSaveCTA && renderOldSaveCTA()}
      </Flex>
      <Flex flexDirection="row" backgroundColor="blue">
        <Join separator={<Spacer x={1} />}>
          {!!showSaveIcon && !!enableRedesignSaveCTA && renderNewSaveCTA()}
          {!!enableAddFollowCTA && renderFollowCTA()}
        </Join>
      </Flex>
    </Flex>
  )
...
const FollowArtistIcon = ({ fill, ...restProps }: IconProps) => {
  ...
  return (
    <Icon {...restProps} viewBox="0 0 18 13">
      <Path
