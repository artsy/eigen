 ...
 * |                                         |
 * |  ------------------------------------   |
 * | | MyProfileEditModalWithSuspense     |  |
 * | |                                    |  |
 * | |  -----------------------------     |  |
...
interface MyProfileEditModalProps {
  source: "inquiry" | "offer" | "order"
  onClose: () => void
}

interface MyProfileEditModalContentProps extends MyProfileEditModalProps {
  me: MyProfileEditModal_me$key
}

export const MyProfileEditModal: React.FC<MyProfileEditModalProps> = ({ source, onClose }) => {
  return (
    <AutomountedBottomSheetModal visible={true} enableDynamicSizing>
      <BottomSheetScrollView>
        <MyProfileEditModalWithSuspense onClose={onClose} source={source} />
      </BottomSheetScrollView>
    </AutomountedBottomSheetModal>
  )
}

const MyProfileEditModalWithSuspense: React.FC<MyProfileEditModalProps> = withSuspense(
  ({ source, onClose }) => {
    ...
    // return <MyProfileEditModalSkeleton />
    return <MyProfileEditModalContent source={source} onClose={onClose} me={data.me} />
  },
  () => <MyProfileEditModalSkeleton />
)

const MyProfileEditModalSkeleton: React.FC = () => {
  return (
    <Skeleton>
      <Box p={2}>
        <Spacer y={0.5} />
...
const MyProfileEditModalContent: React.FC<MyProfileEditModalContentProps> = ({
  ...
  // TODO: what am I suposed to with inProgress?
  const [commit, _inProgress] = useUpdateUserProfileFields()

  // TODO: update cohesion and pass the correct context_screen and context_screen_owner_type
  const tracks = {
  ...
  return (
    <Box p={2}>
      <Text>
        {source === "inquiry"
          ? // TODO: replace Arcadia Contemporary with the correct gallery name
            "Inquiry sent! Tell Arcadia Contemporary more about yourself."
          : "Tell us a few more details about yourself to complete your profile."}
      </Text>
