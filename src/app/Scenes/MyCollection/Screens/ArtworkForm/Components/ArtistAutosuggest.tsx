...
export const ArtistAutosuggest: React.FC<ArtistAutosuggestProps> = ({
  ...
  const filteredCollecteArtists = enableArtworksFromNonArtsyArtists
    ? sortBy(filterArtistsByKeyword(collectedArtists, trimmedQuery), ["displayLabel"]).map((a) => ({
        // We use __typename to properly format arists in the AutosuggestResults component
        __typename: "Artist",
        ...a,
      }))
    : []
            ...
            <AutosuggestResults
              ...
              ListHeaderComponent={() =>
                enableArtworksFromNonArtsyArtists && onlyShowCollectedArtists ? (
                  <Text mb={2}>Artists in My Collection</Text>
                ) : enableArtworksFromNonArtsyArtists ? (
                  <Spacer y={2} />
                ) : null
              }
