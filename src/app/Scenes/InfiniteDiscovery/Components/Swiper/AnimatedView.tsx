...
import { FC, Key, memo, ReactElement, useEffect } from "react"
...
export const AnimatedView: FC<AnimatedViewProps> = memo(
  ({ card, activeCardX, swipedCardX, activeIndex, index, isSwiped }) => {
    ...
    const unswipedCardStyle = useAnimatedStyle(() => {
      if (index <= activeIndex.value - 2 && !isSwiped) {
        return { opacity: 0 }
      }
    ...
    const swipedCardStyle = useAnimatedStyle(() => {
      if (isSwiped && index !== activeIndex.value + 1) {
