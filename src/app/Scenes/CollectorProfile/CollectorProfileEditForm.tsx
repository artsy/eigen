...
interface EditCollectorProfileValuesSchema {
  photo: string
  name: string
...
const editCollectorProfileSchema = Yup.object().shape({
  photo: Yup.string(),
  name: Yup.string().required("Name is required"),
  bio: Yup.string(),
})

export const CollectorProfileEditForm: React.FC = () => {
  const data = useLazyLoadQuery<CollectorProfileEditFormQuery>(
    CollectorProfileEditFormScreenQuery,
    {}
  )

  const [me, refetch] = useRefetchableFragment<
    CollectorProfileEditFormQuery,
    CollectorProfileEditForm_me$key
  >(meFragment, data.me)
  ...
  const { localImage, setLocalImage } = useContext(CollectorProfileContext)
  ...
  const updateUserInfo = async ({
    ...
    bio,
  }: Partial<EditCollectorProfileValuesSchema>) => {
    const payload = {
      name,
      ...(location ? { location } : {}),
      profession,
  ...
  const { handleSubmit, handleChange, setFieldValue, dirty, values, errors, validateForm } =
    useFormik<EditCollectorProfileValuesSchema>({
      ...
      validationSchema: editCollectorProfileSchema,
    })
...
const meFragment = graphql`
  fragment CollectorProfileEditForm_me on Me
  @refetchable(queryName: "CollectorProfileEditForm_meRefetch") {
    name
    profession
    otherRelevantPositions
    bio
...
const CollectorProfileEditFormScreenQuery = graphql`
  query CollectorProfileEditFormQuery {
    me {
