...
import {
  ...
  CollectorProfilePushNotificationsQueryRenderer,
  OpenSettingsBanner,
} from "./CollectorProfilePushNotifications"
...
describe(CollectorProfilePushNotificationsQueryRenderer, () => {
  it("Loads until the operation resolves", () => {
    const tree = renderWithWrappers(<CollectorProfilePushNotificationsQueryRenderer />)
    expect(tree.root.findAllByType(CollectorProfilePushNotifications)).toHaveLength(1)
    expect(tree.root.findByType(CollectorProfilePushNotifications).props.isLoading).toEqual(true)
  })
  ...
  it("renders without throwing an error", () => {
    ...
    expect(env.mock.getMostRecentOperation().request.node.operation.name).toBe(
      "CollectorProfilePushNotificationsQuery"
    )
    ...
    expect(tree.root.findAllByType(CollectorProfilePushNotifications)).toHaveLength(1)
    expect(tree.root.findByType(CollectorProfilePushNotifications).props.isLoading).toEqual(
      undefined
    )
  })
  ...
  it("should show the open settings banner on iOS if the user did not allow push notifications", async () => {
    ...
    tree.update(<CollectorProfilePushNotificationsQueryRenderer />)
