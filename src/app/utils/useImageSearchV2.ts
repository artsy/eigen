import { useImageSearchV2Query } from "__generated__/useImageSearchV2Query.graphql"
...
export const useImageSearchV2 = () => {
  ...
  const handleSeachByImage = async (image: SearchImage) => {
    try {
      ...
      const response = await fetchQuery<useImageSearchV2Query>(
        defaultEnvironment,
        graphql`
          query useImageSearchV2Query($file: Upload!) {
