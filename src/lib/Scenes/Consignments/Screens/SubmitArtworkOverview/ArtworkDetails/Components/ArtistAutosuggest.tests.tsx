...
describe("ArtworkDetailsForm", () => {
  ...
  describe("ArtistAutoSuggest", () => {
    ...
    it("displays full autosuggest result, when at least 3 characters typed", () => {
      const { getByTestId, findByText } = renderWithWrappersTL(<TestRenderer />)
      const artistInput = getByTestId("Submission_ArtistInput")

      act(() => fireEvent.changeText(artistInput, "max"))
      expect(findByText("max ernst")).toBeTruthy()
    })
  })
})
