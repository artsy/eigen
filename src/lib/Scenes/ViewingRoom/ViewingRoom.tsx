      ...
      <Theme>
        <View style={{ flex: 1 }} ref={navRef}>
          <LazyScrollView
            ...
            onVisiblePointsChanged={({ changed }) => {
              if (changed.find(key => key.includes("body"))) {
                trackBodyImpression()
          ...
          >
            ...
            {viewingRoom.status === ViewingRoomStatus.CLOSED || viewingRoom.status === ViewingRoomStatus.SCHEDULED ? (
              <ClosedNotice status={viewingRoom.status} navRef={navRef} partnerHref={viewingRoom.partner!.href!} />
            ) : null}

            {viewingRoom.status === ViewingRoomStatus.LIVE
              ? [
                  <Flex mt="2" mx="2">
                  ...
                  <>
                    <Spacer mb={3} />
                    <ViewingRoomSubsectionsContainer viewingRoom={viewingRoom} />
                  </>,
                ]
              : null}
          </LazyScrollView>
