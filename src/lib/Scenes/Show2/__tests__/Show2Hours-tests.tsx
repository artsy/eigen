import { Show2HoursTestsQuery } from "__generated__/Show2HoursTestsQuery.graphql"
...
describe("Show2Hours", () => {
  ...
  const TestRenderer = () => (
    <QueryRenderer<Show2HoursTestsQuery>
      environment={env}
      query={graphql`
        query Show2HoursTestsQuery($showID: String!) @relay_test_operation {
          show(id: $showID) {
            ...Show2Hours_show
          }
        }
      ...
      render={({ props, error }) => {
        if (props?.show) {
          return <Show2HoursFragmentContainer show={props.show} />
        } else if (error) {
          console.log(error)
        }
      }}
