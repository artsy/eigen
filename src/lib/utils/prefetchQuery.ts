...
const prefetchUrl = async (environment: Environment, url: string, variables: Variables = {}) => {
  ...
  if (module.type !== "react") {
    console.error(`Failed to prefetch ${url} (cannot prefetch native module).`)
    return
  }
...
export const usePrefetch = () => {
  const enablePrefetching = useFeatureFlag("ARPrefetching")
  ...
  return prefetchUrl.bind(this, environment as RelayModernEnvironment)
}
