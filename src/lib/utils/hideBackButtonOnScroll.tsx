...
export const hideBackButtonOnScroll: NonNullable<FlatListProps<any>["onScroll"]> = (ev) => {
  ...
  if (yOffset < SCROLL_DOWN_TO_HIDE_THRESHOLD) {
    LegacyNativeModules.ARScreenPresenterModule.updateShouldHideBackButton(false, unsafe__getSelectedTab())
    return
  }
  ...
  if (direction === "up" && offsetWhenDirectionChanged - yOffset > SCROLL_UP_TO_SHOW_THRESHOLD) {
    ...
    LegacyNativeModules.ARScreenPresenterModule.updateShouldHideBackButton(true, unsafe__getSelectedTab())
    return
  }
}
