...
describe("NativeModel", () => {
  it(`stores the bottom tab state`, async () => {
    expect(__appStoreTestUtils__?.getCurrentState().native.sessionState.selectedTab).toBe("home")
  })
  ...
  it(`responds to native state update events`, () => {
    ...
    expect(__appStoreTestUtils__?.getCurrentState().native.sessionState.selectedTab).toBe("search")
  })
