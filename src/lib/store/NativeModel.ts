...
listenToNativeEvents((event: NativeEvent) => {
  switch (event.type) {
    ...
    case "STATE_CHANGED":
      ...
      if (event.payload.onboardingState) {
        GlobalStore.actions.auth.setState({ userEmail: event.payload.onboardingState })
      }
      if (event.payload.userID) {
        GlobalStore.actions.auth.setState({ userID: event.payload.userID })
      }
      if (event.payload.authenticationToken) {
        GlobalStore.actions.auth.setState({ userAccessToken: event.payload.authenticationToken })
      }
