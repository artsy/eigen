...
// 2. Possibly do sale artwork column layout now, as we can do so based just on the aspect ratio, assuming the text height
//    won't be too different between sale artworks.
// 3. Get sale artwork heights by either:
//    - calculating the item size upfront with aspect ratio and a static height for the text labels.
//    - leting the sale artwork component do a layout pass and calculate its own height based on the column width.
...
import { SaleArtworkGridItemContainer } from "./SaleArtworkGridItem"
...
import { InfiniteScrollSaleArtworksGrid_connection } from "__generated__/InfiniteScrollSaleArtworksGrid_connection.graphql"
 ...
 * - deal with edge-cases when calculating in which section an sale artwork should go
...
export interface Props {
  ...
  /** Pass true if sale artworks should have a Box wrapper with gutter padding */
  shouldAddPadding?: boolean

  /** Defaults to true, pass false to enable fetching more sale artworks via pressing "Show More" button instead of on scroll */
  autoFetch?: boolean
...
interface PrivateProps {
  connection: InfiniteScrollSaleArtworksGrid_connection
...
class InfiniteScrollSaleArtworksGrid extends React.Component<Props & PrivateProps, State> {
  static defaultProps = {
  ...
  sectionedArtworks() {
    ...
    const sectionedArtworks: Array<typeof saleArtworks> = []
    ...
    saleArtworks.forEach((saleArtwork) => {
      ...
      if (saleArtwork.artwork?.image) {
        ...
        if (sectionIndex != null) {
          ...
          section.push(saleArtwork)

          // Keep track of total section aspect ratio
          const aspectRatio = saleArtwork.artwork?.image?.aspectRatio || 1 // Ensure we never divide by null/0
  ...
  renderSections() {
    ...
    for (let i = 0; i < (this.props.sectionCount ?? 0); i++) {
      ...
      for (let j = 0; j < sectionedArtworks[i].length; j++) {
        const saleArtwork = sectionedArtworks[i][j]
        artworkComponents.push(
          <SaleArtworkGridItemContainer
        ...
        if (j < saleArtworks.length - 1) {
          artworkComponents.push(
            <View style={spacerStyle} key={"spacer-" + j + "-" + saleArtwork.id} accessibilityLabel="Spacer View" />
          )
        }
      }
  ...
  render() {
    const saleArtworks = this.state.sectionDimension ? this.renderSections() : null
    ...
    return (
      <Theme>
        <ScrollView
          ...
          scrollsToTop={false}
          accessibilityLabel="Sale Artworks ScrollView"
          stickyHeaderIndices={stickyHeaderIndices}
        >
          {this.renderHeader()}
          <Box px={boxPadding}>
            <View style={styles.container} accessibilityLabel="Sale Artworks Content View">
              {saleArtworks}
            </View>
          </Box>
...
export const InfiniteScrollSaleArtworksGridContainer = createFragmentContainer(InfiniteScrollSaleArtworksGrid, {
  connection: graphql`
