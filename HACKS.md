...
## isDebuggingRemotely check in DevMenu.tsx

#### When can we remove this:

We can't, not until react-native-device-info supports remote debugging. See https://github.com/react-native-device-info/react-native-device-info/issues/776

#### Explanation/Context:

When attempting to fix remote debugging in Chrome, discovered that this library was throwing an error due to Sync apis being used at runtime. By stubbing out this libraries values we were able to fix it, and remote debugging was now available again.
