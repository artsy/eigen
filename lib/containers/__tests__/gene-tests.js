...
describe('state', () => {
  ...
  it('updates from the switch change the selectedTabIndex', ()=> {
      ...
      expect(gene.setState).toHaveBeenLastCalledWith({selectedTabIndex: 23})
  })
  ...
  it('updates the state with new data from Eigen', () => {
    // Setup a Gene Component like normal
     const gene = new Gene()
      ...
      expect(gene.setState).toHaveBeenLastCalledWith({
      ...
      expect(gene.props.relay.setVariables).toHaveBeenLastCalledWith({
        medium: 'porcupines',
