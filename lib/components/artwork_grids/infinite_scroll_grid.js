...
class InfiniteScrollArtworksGrid extends React.Component {
  ...
  fetchNextPage() {
    console.log('this.state.fetchingNextPage', this.state.fetchingNextPage)
    console.log('this.state.completed', this.state.completed)
    if (this.state.fetchingNextPage || this.state.completed) {
      return
    }
    console.log('fetchNextPage', this, this.props.relay.variables.totalSize + PageSize, this.props.filter)
