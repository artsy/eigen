...
class ArtworkRail extends React.Component {
  ...
  renderModuleResults() {
    ...
    if (this.props.rail.results && this.props.rail.results.length) {
      return (
        <View style={[styles.container, { height: this.state.expanded ? this.state.gridHeight + 150 : initialRailHeight }]}>
          <View style={[styles.gridContainer, { height: this.state.expanded ? this.state.gridHeight + 100 : initialInternalViewHeight }]}>
