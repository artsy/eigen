...
class ArtworkRail extends React.Component {
  ...
  geneQueryLink(rail) {
    ...
    // Pull out any params, removing the first (which is `__data_id__`)
    // and turn that into a query
    return rail.context && rail.context.href + '?' +
      Object.keys(rail.params).slice(1).map(function(key) {
  ...
  render() {
    if (this.state.loadFailed !== null && this.state.loadFailed) {
      return null
    }
