...
import { Promise, ScrollView, StyleSheet, View } from 'react-native'
...
import AnimatedCardWrapper from './animated_card_wrapper'
...
class ArtistRail extends React.Component {
  ...
  handleFollowChange(artist) {
    metaphysics(suggestedArtistQuery(artist._id))
      ...
      .then((suggested_artist) => {
        ...
        if (suggested_artist) {
          artists[index]._suggested_replacement = suggested_artist  // trigger replace animation with new suggestion
        } else {
  ...
  handleReplacedEvent(artist) {
    console.log('Replace ' + artist.id + ' by ' + artist._suggested_replacement.id)
    const artists = this.state.artists.slice(0)
    const index = artists.indexOf(artist)
    artists[index] = artist._suggested_replacement
    this.setState({ artists })
  }
  ...
  renderModuleResults() {
    if (this.state.artists.length > 0) {
      return this.state.artists.map(artist => {
        ...
        return <AnimatedCardWrapper key={key}
                                    artist={artist}
                                    suggested_artist={artist._suggested_replacement}
                                    onFollow={() => this.handleFollowChange(artist)}
                                    onCardReplaced={() => this.handleReplacedEvent(artist)} />
      })
    } else {
