...
class ArtworksGrid extends React.Component {
  ...
  render() {
    ...
    return (
      <ScrollView onScroll={this.props.paginate ? this.onScroll : null}
...
const artworksQuery = (artistID, page) => {
  return `
    query {
      artist(id: "${artistID}") {
        artworks(sort: published_at_asc, filter: IS_NOT_FOR_SALE size: 10, page: ${page}) {
          title
          date
          image {
            url(version: "large")
...
export default Relay.createContainer(ArtworksGrid, {
  fragments: {
    artworks: () => Relay.QL`
