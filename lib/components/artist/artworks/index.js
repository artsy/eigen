...
class Artworks extends React.Component {
  ...
  render() {
    ...
    if (for_sale_count === 0) {
      return this.renderSection('Works',
                                ...
                                (artistID, page) => artworksQuery(artistID, 'IS_NOT_FOR_SALE', page))
    } else {
      const otherWorks = []
      ...
      return (
        <View style={styles.section}>
          {this.renderSection('Works for Sale',
                              ...
                              (artistID, page) => artworksQuery(artistID, 'IS_FOR_SALE', page),
  ...
  renderSection(title, count, artworks, query, onComplete) {
    return (
      <View key={title}>
        ...
        <ArtworksGrid artworks={artworks} artworksQuery={query} onComplete={onComplete} />
      </View>
    )
  }
}
