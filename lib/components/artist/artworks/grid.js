...
class ArtworksGrid extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = { sectionDimension: 0, artworks: this.props.artworks, page: 1, completed: false };
  ...
  fetchNextPage() {
    ...
    fetch(metaphysicsURL, { method: 'post',
      ...
      .then(({ data }) => {
        ...
        const completed = artworks < 10;
        if (completed && this.props.onComplete) {
          this.props.onComplete();
        }
        this.setState({ page: nextPage, artworks: this.state.artworks.concat(artworks), completed: completed });
      })
      .catch((error) => {
        console.log(error);
      });
...
ArtworksGridContainer.artworksQuery = (artistID, filter, page) => {
  return `
    query {
      artist(id: "${artistID}") {
        artworks(sort: published_at_asc, filter: ${filter} size: 10, page: ${page}) {
          title
          date
          image {
            url(version: "large")
