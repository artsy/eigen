  ...
  render() {
    ...
    return (
      <View style={{marginLeft: sideMargin, marginRight: sideMargin}}>
        ...
        { this.blurb(artist) }
        <SerifText style={{ marginBottom: 40, fontSize: 16, lineHeight: 20 }} numberOfLines={0}>{this.bioText()}</SerifText>
      </View>
    );
  }

  blurb(artist) {
    return artist.blurb ? <SerifText style={{ marginBottom: 20, fontSize: 16, lineHeight: 20 }} numberOfLines={0}>{removeMarkdown(artist.blurb)}</SerifText> : null;
  }
