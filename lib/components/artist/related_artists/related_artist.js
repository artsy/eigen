...
class RelatedArtist extends React.Component {
  ...
  render() {
    ...
    const imageWidth =  !isPhone ? isPadHorizontal ? ((window.width - 100) / 4) : ((window.width - 100) / 3) : ((window.width - 60) / 2);
    console.log(imageWidth);
    const imageHeight = Math.floor(imageWidth / 1.5);
    
    if (!artist) {
      return (<View style={{margin: 5, paddingBottom: 20, width: imageWidth}}/>)
    }
