...
class RelatedArtist extends React.Component {
  ...
  imageDimensions() {
    ...
    const imageWidth = (!isPhone ? (isPadHorizontal ? (window.width - 140)/4 : (window.width - 100)/3) : ((window.width - 60)/2))
  ...
  render() {
    ...
    if (!artist) {
      return (<View style={{margin: 5, paddingBottom: 20, width: this.state.width}}/>)
    }

    const imageURL = artist.image && artist.image.url;

    return (
      <TouchableWithoutFeedback onPress={this.handleTap.bind(this)} onLayout={this.onLayout.bind(this)}>
        <View style={{ margin: 5, paddingBottom: 20, width: this.state.width }}>
          <ImageView style={{ width: this.state.width, height: this.state.height }} imageURL={imageURL} />
