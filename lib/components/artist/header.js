...
class Header extends React.Component {
  ...
  render() {
    ...
    return (
      <View>
        ...
        {this.renderSection(artist.nationality, artist.birthday)}
  ...
  renderSection(nationality, birthday) {
    const bylineRequired = (nationality || birthday);
    const nationalityString = nationality + (birthday ? ", " : "");
    const birthdayString = "b. " + birthday;
    if (bylineRequired) {
      return ( 
        <View>
            <SerifText style={styles.base}>
                {nationality ? nationalityString : null}{birthday ? birthdayString : null}
              </SerifText>
        </View>
      );
    } else {
