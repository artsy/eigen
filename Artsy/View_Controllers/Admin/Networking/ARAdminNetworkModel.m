...
- (void)downloadJavaScriptForMainCommit:(void (^ _Nonnull)(NSString * _Nullable title, NSString * _Nullable subtitle))metadataCallback completion:(void (^_Nonnull)(NSURL * _Nullable downloadedFileURL, NSError * _Nullable error))completionHandler;
{
    // Get the metadata first, then grab the master commit if we get metadata correctly
    [self downloadMetadataForMainCommit:^(NSError * _Nullable error, Metadata *metadata) {
