...
- (id)initWithSelection:(BOOL)selected
{
    ...
    if (self) {
        self.backgroundColor = [[UIColor artsyColorFor:@"black10"] colorWithAlphaComponent:0.2];
...
- (void)drawStretchyRectWithPointsTL:(CGPoint)TL TR:(CGPoint)TR BL:(CGPoint)BL BR:(CGPoint)BR inContext:(CGContextRef)ctx
{
    ...
    // Color it
    CGContextSetFillColorWithColor(ctx, [UIColor artsyColorFor:@"devpurple"].CGColor);
